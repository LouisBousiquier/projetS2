...
class VertexInterface
{
    ...
    public :
        ...
        VertexInterface(std::string nom, int idx, int x, int y ,int minim,int maxi, std::string pic_name="", int pic_idx=0);
};
...
class Vertex
{
    ...
    public:
        ...
        ///louis
        void Print(std::ofstream &stream,char separator,char secondarySeparator) const;
        ///end louis

        double getval();
        void setval(double nouvelleval);
};
...
class Graph
{
    ...
    public:
        ...
        Graph (GraphInterface *interface=nullptr) :
            m_interface(interface), m_a(-1)  {  }

        void add_interfaced_vertex(std::string nom, int idx, double value, int x, int y, int minim, int maxi, std::string pic_name="", int pic_idx=0);
        void add_interfaced_edge(int idx, int vert1, int vert2, double weight=0);

        ///methodes pour les graphes
        void graphe1();
        void graphe2();
        ///void afficherPoidsSommet();
        ///void changeValSommet();
        ...
        ///méthode pour enlever une arete
        void test_remove_edge(int eidx);

        ///louis///////
        ///Enregistre et charge un graphe complet
        void Enregistrer(std::string fichier);
        void Charger(std::string fichier);

        ///Ecrit vertex et Edges
        ///C'est fait dans la classe Graph pour bénéficier du friend
        void PrintVertex(std::ofstream & outfile,const Vertex &  v);
        void PrintEdges(std::ofstream & outfile,const Edge & e);
        ///separator to parse the file
        char m_separator = ' ';///attribute separator (séparateur entre chaque attribut d'un vertex/Edge)
        char m_listSeparator = ';';///inclusive list separaor (séparateur entre chaque valeur d'une liste d'un attribut (m_in,m_out,etc)
        std::string m_vertexName = "Vertex";
        std::string m_edgesName = "Edges";
        void CreateInterface();///create the interface je ne sauvegarde pas l'interface dans le fichier donc il faut la créer
        ///end louis
};
